  ...
  return (
    <div className="page" data-testid="artist_screen">
      <div className="banner">
        <img src={artistPicture(a?.picture, 750)} alt="" fetchPriority="high" />
      ...
      <div className="artist__actions">
        ...
        <span style={{ fontSize: 12, color: 'var(--subdued)', marginLeft: 4 }}>{[counts.ALBUM && `${counts.ALBUM} albums`, counts.EP && `${counts.EP} EPs`, counts.SINGLE && `${counts.SINGLE} singles`].filter(Boolean).join(' · ')}</span>
      ...
      <Section title="Discography">
        <div className="chips" role="tablist">{(['ALL', 'ALBUM', 'EP', 'SINGLE'] as const).map((f) => <button key={f} role="tab" aria-selected={filter === f} className={`chip${filter === f ? ' is-on' : ''}`} onClick={() => setFilter(f)}>{f === 'ALL' ? 'All releases' : f === 'ALBUM' ? 'Albums' : f === 'EP' ? 'EPs' : 'Singles'}</button>)}</div>
